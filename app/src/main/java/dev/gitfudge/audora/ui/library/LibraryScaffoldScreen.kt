...
import dev.gitfudge.audora.ui.components.ScopeItem
import dev.gitfudge.audora.ui.components.ScopeToolbar
    ...
    if (bulkEditorOpen) {
        if (combineMode) {
            val plan = remember(bulkEditorTracks) {
                dev.gitfudge.audora.domain.buildCombinePlan(bulkEditorTracks)
            }
            dev.gitfudge.audora.ui.common.BulkTagEditorSheet(
                ...
                prefill = buildMap {
                    if (plan.canonicalAlbum.isNotBlank()) {
                        put(dev.gitfudge.audora.domain.BulkTagField.ALBUM, plan.canonicalAlbum)
                    }
                    put(
                        dev.gitfudge.audora.domain.BulkTagField.ALBUM_ARTIST,
...
private fun LibraryScaffoldContent(
    ...
    albums: List<dev.gitfudge.audora.domain.AlbumSummary>,
    unfiledCount: Int,
    onTabChange: (dev.gitfudge.audora.domain.LibraryTab) -> Unit,
    onAlbumClick: (albumKey: String) -> Unit,
    onUnfiledClick: () -> Unit,
        ...
        bottomBar = {
            ...
            AnimatedVisibility(
                ...
                SelectionBar(
                    ...
                    combineEnabled = tab == dev.gitfudge.audora.domain.LibraryTab.ALBUMS &&
        ...
        Column(
            ...
            Row(
                ...
                ScopeToolbar(
                    ...
                    selectedIndex = if (tab == dev.gitfudge.audora.domain.LibraryTab.ALBUMS) 0 else 1,
                    onSelected = { idx ->
                        onTabChange(
                            if (idx == 0) dev.gitfudge.audora.domain.LibraryTab.ALBUMS
                            else dev.gitfudge.audora.domain.LibraryTab.TRACKS,
                        )
                    },
                )
            ...
            when {
                tab == dev.gitfudge.audora.domain.LibraryTab.TRACKS &&
                ...
                tab == dev.gitfudge.audora.domain.LibraryTab.ALBUMS -> AlbumsPane(
                    albums = albums,
                    unfiledCount = unfiledCount,
                    onAlbumClick = onAlbumClick,
                    onUnfiledClick = onUnfiledClick,
                ...
                Row(
                    ...
                    when {
                        ...
                        batchArtFetchState is BatchFetchState.Running -> {
                            ...
                            dev.gitfudge.audora.ui.components.GhostButton(
                                onClick = onEditTags,
                                modifier = Modifier.weight(1f),
                            ) {
...
@Composable
private fun CombineHeader(plan: dev.gitfudge.audora.domain.CombinePlan) {
    val colors = MaterialTheme.colorScheme
    val spacing = LocalSpacing.current
